...
| A page request's Scope closes on every exit but a returned body                             | `packages/effect-frame/tests/router/respond-document.test.ts`: "a defect closes the Scope and answers 500", "a redirect answers 303 with the path and search, and closes the Scope", "a document keeps its Scope open until the body ends"                                                                                    | `document.ts` `respondDocument` does not close the Scope on a non-success exit                                    | Killed (the defect test), when it lived in the dashboard as `answerWith` |
...
14. **The page request's Scope is closed on every exit but a returned
    body.** The framework's `respondDocument` closes it on a failure, a
    defect or an interruption while drawing or while making the response,
    and answers 500 on a defect. A returned body keeps it until the body
    ends.
