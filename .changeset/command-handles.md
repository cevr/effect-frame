...
Remote commands claim their contract's live query entries, which stay stale until the last command settles and refresh after it applies. The claim is private to the cache that `queryCacheLayer` builds: `QueryCacheService` is unchanged, and a custom cache implementation keeps working without command ownership. `Frame.inspect` now reports `commands` as `Available` with its retained records.
