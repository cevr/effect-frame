...
| A predicted add the host rejects is taken back    | `command.test.tsx`: "an add to the read-only list is predicted, then the host's policy refuses it and the row is taken back" | The `notes` policy allows every send                                                            | Killed                                                                            |
| (same row)                                        | (same test)                                                                                                                  | `provisional.ts` keeps an overlay that leaves without a receipt                                 | Killed                                                                            |
...
6. **The behavior refuses `reject-me`.** #25 §1 says the behavior refuses
   ...
   longer fakes a rejection; it only records the host's own `Refused`.
   The take-back of a predicted row needs a rejection the page cannot
   foresee, so Notes has one (review round 2): the contract names the
   policy `notes`, and the server's rule (`policies.server.ts`) lets any
   caller read any list but refuses a send to the read-only list
   `archive` with `Unauthorized`. The rule is server-only, so the page
   predicts the add; the real host refuses it before admission, the
   command owner takes the refusal as conclusive, and the predicted row
   leaves the pending log. "an add to the read-only list is predicted,
   then the host's policy refuses it and the row is taken back" proves it
   through the real host. The wiretap records the policy's `Unauthorized`
   as it records `Refused`.
