...
| L3 | CI on 11c3842 failed once: transport test 3's SIGINT'd reader exited 130 with no `interrupted` on stderr (the default signal exit), while the Version PR run on the same code passed and 7 local runs passed. Suspected cause, not proven: `bin.ts` forked the signal listener with `forkChild`, which schedules the child, so "listen from the start" rested on scheduler order. It now forks with `startImmediately: true`: the listeners are on the process before `forkChild` returns | explicit | `packages/inspect/src/bin.ts` | 1 | low | hardened; flake not reproduced |
