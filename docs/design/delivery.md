...
| ----------------------------------- | --------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `effect-frame/actor`                | `.`             | server         | everything below plus `Actor.durable`, `MailboxStore`, `implement`, `ActorHost`, `HttpServer`                                                     |
| `effect-frame/actor`                | `./client`      | browser        | vocabulary, `contract`, `resumeCodec`, `Actor.local` and `Behavior`, `Actor.remote` and `Actor.remoteCommands`, `ActorTransport`, `HttpTransport` |
