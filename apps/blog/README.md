...
```sh
bun run prerender   # build dist/prerender from posts/
bun run dev         # serve it on http://127.0.0.1:3000
```

`bun run start` does both. `bun run build` bundles the client only: the gate runs `build`, and the gate never reads content (#23 §2.6). From the repository root, `bun run prerender` runs the same build.
