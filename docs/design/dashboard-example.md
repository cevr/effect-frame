...
| An override is dropped by any authoritative value and never rolled back on `Rejected` (#19) | `query.test.tsx`: "the override shows at once, stale, and the ack's refresh replaces it", "any authoritative value drops the override, before the ack lands", "a Rejected ack leaves the override; the next authoritative value replaces it" | `commands.ts` `ack` does not call `override`                                              | Killed (3 tests)                                  |
...
10. **`Alerts` is a machine, and an `Ack` writes through `override`.** The
    ...
    the header's `TenantInfo` entry through `override`, then sends. One
