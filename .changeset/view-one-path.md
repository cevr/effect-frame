...
- `ViewTest` moves off `effect-frame/view` to its own subpath, as `QueryTest` is on `effect-frame/actor/testing`: `import { ViewTest } from "effect-frame/view/testing"`. A browser entry that imports `effect-frame/view` no longer carries the test harness.
