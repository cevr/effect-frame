...
- `props.outlet` is an `Effect<Node, never, ChildR>`. It is a delayed setup.
  ...
  the layout yields it outside `View.loading`, `View.LoadingScope` stays in the branch's
  requirements and the mode constructor refuses the tree: `View.ready needs a
View.loading above it` (`ScopesClosed`, type fixture).
...
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Route data types are exact                            | 0. "keeps exact route data and requirement types"                                                  | The route's services equal `QueryCache \| ActorTransport`. The inherited binding is a `FollowedQuery`. The actor binding is `{ ref: Source<RemoteActorRef<typeof Draft>>; state: Source<string> }`. A leaky outlet keeps `LoadingScope` in the branch, and the mode constructor refuses it. A name collision, a missing binding, and a ref used as a function do not compile.                                                                                                                                                   |
