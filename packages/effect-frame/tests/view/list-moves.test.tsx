...
import { Dom, For, View, mount, render } from "effect-frame/view";
...
describe("a keyed list moves only what moved", () => {
  it.scoped("a change that keeps every row in place touches no node", () =>
    Effect.gen(function* () {
      ...
      yield* mount(Keyed, { items: items.state }, host, root);
      ...
      yield* items.call(Value.Set(["a", "b", "c"]));
      yield* render;
  ...
  it.scoped("moving one row to the front inserts only that row", () =>
    Effect.gen(function* () {
      ...
      yield* items.call(Value.Set(["c", "a", "b"]));
      yield* render;
