...
7. **The build is `Anonymous`, and a failed read fails the build.** The
   ...
   `PrerenderUnauthorized { route, href, query }`; `query` names the query
   contract, which names its policy. Another failed read fails with
...
| A policy that refuses `Anonymous` fails the build         | `prerender-build.test.tsx` — "a query whose policy refuses Anonymous fails the build with PrerenderUnauthorized and writes no file"                                                                                        |
