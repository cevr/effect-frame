...
`Presentation`, `presentWith`, `settleLoading`, the ticket and timer in
...
4. The fallback is drawn at `start + after`, unless the setup already
   finished. A result that is already there wins over a deadline that has
   already passed. Work that finishes sooner never draws the fallback.
5. Once drawn, a successful setup's node replaces it at `shown + atLeast` at
   the earliest. `shown` is the deadline at which it was drawn.
...
9. While the instance prepares, its region registers one settled read with
   ...
   fallback, not its own, and not before `after`. The registration ends when
   the view's node is drawn; the view's own reads, made during its setup,
   then decide that `Loading`.
...
5. The platform import runs on a detached fiber. An interrupted waiter loses
...
| 3. "a typed import rejection shows errored at once, and a later attempt imports again" | With the fallback shown, the chunk fails: `Setup:LazyImportFailed` shows without any clock movement, and no setup ran. The next navigation enters the failed instance again and imports a second time, then shows the view.                                                                                                                                                                                                                                        |
...
| 7. "the initial first frame waits for the import and setup, and never shows pending"   | The HTML mount does not return while the import is held, even after one second of test clock. After the import, the post's setup has run when the mount returns. The serialized frame has the layout and no pending fallback. Close leaks nothing.                                                                                                                                                                                                                 |
...
| --------------------------------------------------------- | ------------- | ---------------------------------------------------- |
| M1: ignore `after` (draw the fallback at the start)       | 1, 2          | the fallback was in the document at 99ms             |
| M2: ignore `atLeast`                                      | 1             | the fallback was gone at 399ms                       |
...
| M7: do not settle the enclosing `Loading` while preparing | 1, 2, 3, 4, 5 | `ConditionNotObserved`: the presentation region      |
...
- Should declared actor acquisition move inside the pending window? That
  needs a transition that installs an entered instance before its
  declarations settle, while it keeps the old interests until the new ones
  are held and still reports an unhandled acquisition failure truthfully.
  Chosen now: no. Acquisition stays above presentation, as the scope's
  layout rule requires, and the old page stays during it.
